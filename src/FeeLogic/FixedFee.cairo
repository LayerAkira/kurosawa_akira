...
fn apply_fixed_fee_involved(ref state: ContractState, user: ContractAddress, fixedFee: FixedFee, feeable_qty: u256) {
    if fixedFee.pbips == 0 {
        return;
    }
    let fee = (feeable_qty * fixedFee.pbips - 1) / 1000000 + 1;
    if fixedFee.external_call {
        IERC20Dispatcher { contract_address: fixedFee.fee_token }.transferFrom(user, fixedFee.recipient, fee);
    }
    else {
...
impl ZeroableImpl of Zeroable<FixedFee> {
    ...
    fn zero(self: FixedFee) -> FixedFee {
        ...
        FixedFee{recipient: zero_adress, fee_token: zero_adress, pbips: zero_u256, external_call: false}
    }
}